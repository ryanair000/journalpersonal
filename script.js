...
// Keep visible cards in sync with saved data without refreshing the whole page.
const readLiveArray = (key) => {
  try {
...
const replaceLiveArray = (target, values) => target.splice(0, target.length, ...values);
const removeLiveRows = (selector) => document.querySelectorAll(selector).forEach((row) => row.remove());
const appendLiveHtml = (selector, html) => document.querySelector(selector)?.insertAdjacentHTML('beforeend', html);
...
window.addEventListener('mll:data-changed', (event) => {
  ...
  if (detail.source === 'form') {
    if (detail.logType) {
      const button = document.querySelector(`.log-pill[data-log="${CSS.escape(detail.logType)}"]`);
      if (button) { button.textContent = '✓ logged'; button.style.background = 'var(--coral)'; button.style.color = '#fff'; }
    }
    if (triggerId === 'addTask') appendLiveHtml('.schedule-card .module-link', '');
    if (triggerId === 'addTask') {
      const row = document.createElement('div'); row.className = 'focus-task';
      row.innerHTML = `<span class="task-dot"></span><div><strong>${escapeText(values.text)}</strong><small>New focus item</small></div><button aria-label="More options">···</button>`;
      document.querySelector('.schedule-card .module-link')?.before(row);
    }
    if (['addContent', 'overviewIdea', 'addDraft'].includes(triggerId)) {
      const draft = triggerId === 'addDraft'; const item = document.createElement('div'); item.className = 'idea-item'; item.dataset.contentIdea = 'true';
      item.innerHTML = `<span>${draft ? '◒' : '✦'}</span><div><strong>${escapeText(values.text)}</strong><small>${draft ? 'New draft · not scheduled' : 'New idea · choose a platform'}</small></div><button>···</button>`;
      document.querySelector(draft ? '.content-board:nth-of-type(2)' : '.content-board:first-of-type')?.append(item);
    }
    if (triggerId === 'addPlanner') appendLiveHtml('#plannerList', `<p><span>□</span> ${escapeText(values.text)} <small>New task</small></p>`);
    if (['addSocial', 'addSocialLink'].includes(triggerId)) appendLiveHtml('#socialList', `<p><span>♡</span> ${escapeText(values.text)} <small>New plan</small></p>`);
    if (['addMemory', 'overviewMemory'].includes(triggerId)) { const memory = document.querySelector('#memoryText'); if (memory) memory.textContent = values.memory; }
    if (detail.toolType) appendLiveHtml('#relationshipItems', `<p>${escapeText(detail.toolType)}: ${escapeText(values.text)}</p>`);
    if (detail.examUnit) {
      const row = [...document.querySelectorAll('#examPrepList article')].find((item) => item.querySelector('strong')?.textContent.trim() === detail.examUnit);
      const button = row?.querySelector('[data-exam-next30b]'); if (button) button.textContent = 'Next action saved ✓';
    }
  }

  if (keys.has('contentAnalytics')) {
    const data = JSON.parse(localStorage.getItem('contentAnalytics') || '{}');
    if (document.querySelector('#reachMetric')) document.querySelector('#reachMetric').textContent = data.reach || '0';
    if (document.querySelector('#engagementMetric')) document.querySelector('#engagementMetric').textContent = data.engagement || '0%';
    if (document.querySelector('#viewsMetric')) document.querySelector('#viewsMetric').textContent = data.views || '0';
  }
  if (keys.has('weeklyExpenses')) {
    const amount = Number(localStorage.getItem('weeklyExpenses') || 0);
    if (document.querySelector('#spentTotal')) document.querySelector('#spentTotal').textContent = `KSh ${amount.toLocaleString()}`;
    if (document.querySelector('#monthlyMoney')) document.querySelector('#monthlyMoney').textContent = `KSh ${amount.toLocaleString()}`;
  }
  if (keys.has('categoryExpenses')) {
    const fresh = JSON.parse(localStorage.getItem('categoryExpenses') || '{}');
    Object.keys(categoryExpenses).forEach((key) => delete categoryExpenses[key]); Object.assign(categoryExpenses, { food: 0, transport: 0, school: 0, personal: 0 }, fresh); renderCategoryExpenses();
  }
  if (keys.has('calendarEvents')) { calendarEvents = readLiveArray('calendarEvents'); renderCalendar(); renderUpcoming(); }
  if (keys.has('visionItems')) { visionItems = readLiveArray('visionItems'); renderVision(); }
  if (keys.has('customHabits')) { customHabits = readLiveArray('customHabits'); renderCustomHabits(); }
  if (keys.has('boardProjects')) { boardProjects = readLiveArray('boardProjects'); removeLiveRows('[data-project]'); renderBoardProjects(); }
  if (keys.has('archiveEntries')) { replaceLiveArray(archiveEntries, readLiveArray('archiveEntries')); renderArchiveEntries(); }
  if (keys.has('mealLogs')) { replaceLiveArray(mealLogs, readLiveArray('mealLogs')); renderMealLogs(); }
  if (keys.has('scheduleEntries')) { replaceLiveArray(scheduleEntries, readLiveArray('scheduleEntries')); renderScheduleEntries(); }
  if (keys.has('studySessions')) renderStudySessions();
  if (keys.has('savingsGoals')) { savingsGoals = readLiveArray('savingsGoals'); renderSavings(); }

  if (keys.has('contentAccounts')) {
    removeLiveRows('#accountList .saved-account-row');
    readLiveArray('contentAccounts').forEach((account) => appendLiveHtml('#accountList', `<div class="account-row saved-account-row"><span class="platform-icon tiktok">✦</span><div><strong>${escapeText(account.platform)}</strong><small>${escapeText(account.username)}</small></div><b>${escapeText(account.followers)}</b><button class="account-remove" aria-label="Remove account">×</button></div>`));
  }
  if (keys.has('customUnits')) {
    removeLiveRows('#unitList .saved-custom-row');
    readLiveArray('customUnits').forEach((item) => appendLiveHtml('#unitList', `<div class="saved-custom-row"><span>${escapeText(item.code)}</span><strong>${escapeText(item.name)}</strong><small>${escapeText(item.lecturer)} · ${escapeText(item.year)}</small></div>`));
  }
  if (keys.has('schoolStudyItems')) {
    removeLiveRows('#studyList .saved-custom-row');
    readLiveArray('schoolStudyItems').forEach((item) => appendLiveHtml('#studyList', `<div class="saved-custom-row"><span>${escapeText(item.icon || '◒')}</span><strong>${escapeText(item.value)}</strong><small>${escapeText(item.meta)}</small></div>`));
  }
  if (keys.has('schoolResearchItems')) {
    removeLiveRows('#researchList .saved-custom-row');
    readLiveArray('schoolResearchItems').forEach((item) => appendLiveHtml('#researchList', `<p class="saved-custom-row"><span>${escapeText(item.icon || item.type || 'NOTE')}</span> ${escapeText(item.value)} <small>${escapeText(item.meta)}</small></p>`));
  }
  if (keys.has('schoolProjectDetails')) {
    const item = JSON.parse(localStorage.getItem('schoolProjectDetails') || '{}');
    if (document.querySelector('#projectTitle')) document.querySelector('#projectTitle').textContent = item.title || 'Your project';
    if (document.querySelector('#projectNotes')) document.querySelector('#projectNotes').textContent = item.next || 'Add the next action.';
  }
  if (keys.has('classEntries')) {
    const host = document.querySelector('.schedule-entry-list') || (() => { const list = document.createElement('div'); list.className = 'schedule-entry-list'; document.querySelector('.timetable-card')?.append(list); return list; })();
    host.replaceChildren(); readLiveArray('classEntries').forEach((item) => appendLiveHtml('.schedule-entry-list', `<p><span>${escapeText(item.day)}</span><strong>${escapeText(item.subject)}</strong><small>${escapeText(item.time)}</small></p>`));
  }
  if (keys.has('personalBusinesses')) {
    removeLiveRows('#businessList .saved-custom-row');
    readLiveArray('personalBusinesses').forEach((item) => appendLiveHtml('#businessList', `<div class="saved-custom-row"><span class="business-badge coral-badge">${escapeText(item.name.slice(0, 2).toUpperCase())}</span><section><strong>${escapeText(item.name)}</strong><small>${escapeText(item.type)} · ${escapeText(item.duration)}</small></section><b>New</b></div>`));
  }
  if (keys.has('workGoals')) {
    removeLiveRows('#workGoalList .saved-work-goal');
    readLiveArray('workGoals').forEach((item, index) => appendLiveHtml('#workGoalList', `<label class="saved-work-goal"><input type="checkbox" data-saved-work-goal="${index}" ${item.done ? 'checked' : ''}> ${escapeText(item.title)}</label>`));
  }
  if (keys.has('peopleDirectory')) {
    removeLiveRows('#peopleList .saved-custom-row');
    readLiveArray('peopleDirectory').forEach((item) => appendLiveHtml('#peopleList', `<div class="saved-custom-row"><span class="person-avatar peach">${escapeText(item.name.charAt(0).toUpperCase())}</span><section><strong>${escapeText(item.name)}</strong><small>${escapeText(item.group)} · ${escapeText(item.note || 'Keep in touch')}${item.birthday ? ` · ${escapeText(item.birthday)}` : ''}</small></section><button>Check in →</button></div>`));
  }
  if (keys.has('examPrepItems')) {
    removeLiveRows('#examPrepList .saved-custom-row');
    readLiveArray('examPrepItems').forEach((item) => appendLiveHtml('#examPrepList', `<article class="saved-custom-row"><div><strong>${escapeText(item.unit)}</strong><small>${escapeText(item.code)}</small></div><div class="prep-bar"><i style="width:${Number(item.progress) || 0}%"></i></div><b>${Number(item.progress) || 0}%</b></article>`));
  }
  if (keys.has('businessKpis')) {
    removeLiveRows('#businessKpiList .saved-custom-row');
    readLiveArray('businessKpis').forEach((item) => appendLiveHtml('#businessKpiList', `<article class="saved-custom-row"><p class="eyebrow">${escapeText(item.business)}</p><strong>${escapeText(item.value)}</strong><small>${escapeText(item.metric)}</small><span>${escapeText(item.note)}</span></article>`));
  }
  if (keys.has('careerTasks')) {
    removeLiveRows('#careerTaskList .saved-career-row');
    readLiveArray('careerTasks').forEach((item, index) => appendLiveHtml('#careerTaskList', `<label class="saved-career-row"><input type="checkbox" data-saved-career-task="${index}" ${item.done ? 'checked' : ''}> ${escapeText(item.title)}</label>`));
  }
  if (keys.has('pharmacyResources')) {
    removeLiveRows('#resourceList .saved-resource-row'); replaceLiveArray(savedResources, readLiveArray('pharmacyResources')); savedResources.forEach((item) => appendResource(item));
  }
  if (keys.has('examEntries')) {
    const list = document.querySelector('#examList'); if (list) { list.replaceChildren(); readLiveArray('examEntries').forEach((item) => appendLiveHtml('#examList', `<div><span class="exam-code">${escapeText(item.code)}</span><section><strong>${escapeText(item.name)}</strong><small>Exam: ${escapeText(item.date)} · ${escapeText(item.time || 'Time to confirm')}</small>${item.venue ? `<small>Venue: ${escapeText(item.venue)}</small>` : ''}</section><b>${escapeText(item.date)}</b></div>`)); }
  }
  if (keys.has('pipelinePosts')) {
    removeLiveRows('.pipeline-post'); replaceLiveArray(savedPipelinePosts, readLiveArray('pipelinePosts')); savedPipelinePosts.forEach((item) => appendPipelinePost(item));
  }
  if (keys.has('dueItems')) {
    removeLiveRows('#dueList .saved-due-row'); replaceLiveArray(savedDueItems, readLiveArray('dueItems')); savedDueItems.forEach((item) => addDueRow(item));
  }
  if (keys.has('routines')) {
    removeLiveRows('#routineList .saved-routine-row'); readLiveArray('routines').forEach((item) => appendLiveHtml('#routineList', `<article class="saved-routine-row"><span class="routine-icon">↻</span><div><strong>${escapeText(item.name)}</strong><small>${escapeText(item.detail)}</small></div><b>${escapeText(item.time)}</b><input type="checkbox"></article>`));
  }
  if (keys.has('relationshipItems') && detail.source !== 'form') {
    const host = document.querySelector('#relationshipItems'); if (host) { host.replaceChildren(); readLiveArray('relationshipItems').forEach((item) => appendLiveHtml('#relationshipItems', `<p>${escapeText(item.type)}: ${escapeText(item.text)}</p>`)); }
  }
});
...
window.addEventListener('load', () => {
  ...
  readList('examPrepItems').forEach((item) => appendHtml('#examPrepList', `<article class="saved-custom-row"><div><strong>${escapeText(item.unit)}</strong><small>${escapeText(item.code)}</small></div><div class="prep-bar"><i style="width:${item.progress}%"></i></div><b>${item.progress}%</b></article>`));
  readList('businessKpis').forEach((item) => appendHtml('#businessKpiList', `<article class="saved-custom-row"><p class="eyebrow">${escapeText(item.business)}</p><strong>${escapeText(item.value)}</strong><small>${escapeText(item.metric)}</small><span>${escapeText(item.note)}</span></article>`));
  readList('careerTasks').forEach((task, index) => appendHtml('#careerTaskList', `<label class="saved-career-row"><input type="checkbox" data-saved-career-task="${index}" ${task.done ? 'checked' : ''}> ${escapeText(task.title)}</label>`));
...
window.addEventListener('load', () => {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (action.classList.contains('delete-persisted')) { if (!confirm('Delete this saved item?')) return; items.splice(index, 1); localStorage.setItem(key, JSON.stringify(items)); window.dispatchEvent(new CustomEvent('mll:data-changed', { detail: { source: 'legacy-edit', keys: [key] } })); return; }
    ...
    prompts.forEach(([field, label, current]) => { const value = prompt(`${label}:`, current); if (value !== null && value.trim() !== '') item[field] = field === 'progress' ? Math.max(0, Math.min(100, Number(value) || 0)) : value.trim(); }); localStorage.setItem(key, JSON.stringify(items)); window.dispatchEvent(new CustomEvent('mll:data-changed', { detail: { source: 'legacy-edit', keys: [key] } }));
  });
...
window.addEventListener('load', () => {
  ...
  const decorateKpisFifteen = () => {
    ...
    customRows.forEach((row, index) => {
      ...
      controls.addEventListener('click', (event) => {
        ...
        localStorage.setItem('businessKpis', JSON.stringify(current)); window.dispatchEvent(new CustomEvent('mll:data-changed', { detail: { source: 'legacy-edit', keys: ['businessKpis'] } }));
      });
    });
  };
  decorateKpisFifteen();
  ...
  importantListFifteen?.addEventListener('click', (event) => {
    ...
    Object.assign(item, { title: title.trim(), date: date.trim(), person: person?.trim() || 'Personal reminder' }); localStorage.setItem('importantDates', JSON.stringify(dates)); const row = button.closest('.important-date-row'); if (row) { row.querySelector('strong').textContent = item.title; row.querySelector('small').textContent = item.person; row.querySelector('time').textContent = item.date; }
  });
...
window.addEventListener('load', () => {
  ...
  const examActions30b = document.querySelectorAll('#examPrepList article'); const examNext30b = JSON.parse(localStorage.getItem('examNextActions') || '{}'); const examPlans30b = JSON.parse(localStorage.getItem('examActionPlans') || '{}'); examActions30b.forEach((row) => { if (row.querySelector('[data-exam-next30b]')) return; const unit = row.querySelector('strong')?.textContent || ''; const button = document.createElement('button'); button.type = 'button'; button.dataset.examNext30b = 'true'; button.dataset.examUnit30b = unit; button.className = 'exam-next30b'; button.textContent = examNext30b[unit] || 'Add next action'; row.append(button); const plan = examPlans30b[unit]; if (plan) { const detail = document.createElement('small'); detail.className = 'exam-action-detail30b'; detail.textContent = [plan.method, plan.duration ? `${plan.duration} min` : '', plan.targetDate || '', plan.priority ? `${plan.priority} priority` : ''].filter(Boolean).join(' · '); row.append(detail); } });
...
window.addEventListener('load', () => {
  ...
  const milestoneList30 = document.querySelector('#projectMilestonesNext .project-milestone-list'); const projectItems30 = readNext30('projectMilestones'); const decorateMilestoneEdit30 = () => { milestoneList30?.querySelectorAll('label').forEach((row, index) => { if (row.querySelector('[data-milestone-edit30]')) return; const edit = document.createElement('button'); edit.type = 'button'; edit.dataset.milestoneEdit30 = index; edit.textContent = 'Edit'; edit.className = 'milestone-edit30'; row.append(edit); edit.addEventListener('click', (event) => { event.preventDefault(); const title = prompt('Milestone:', projectItems30[index]?.title); if (!title?.trim()) return; projectItems30[index].title = title.trim(); localStorage.setItem('projectMilestones', JSON.stringify(projectItems30)); const copy = [...row.childNodes].find((node) => node.nodeType === Node.TEXT_NODE); if (copy) copy.textContent = ` ${projectItems30[index].title} `; }); }); }; decorateMilestoneEdit30(); if (milestoneList30) new MutationObserver(decorateMilestoneEdit30).observe(milestoneList30, { childList: true });
